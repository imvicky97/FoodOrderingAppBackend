...
//    //This test case passes when you are able to login successfully.
    ...
    public void shouldLoginForValidRequest() throws Exception {
        ...
        when(mockCustomerService.authenticate("9090909090", "qawsedrf@123"))
                .thenReturn(createdCustomerAuthEntity);

        mockMvc
                .perform(post("/customer/login")
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                        .header("authorization", "Basic " + getEncoder().encodeToString("9090909090:qawsedrf@123".getBytes())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("id").value(customerId))
                .andExpect(header().exists("access-token"));
        verify(mockCustomerService, times(1)).authenticate("9090909090", "qawsedrf@123");
    }
...
//
    ...
    public void shouldUpdateCustomerPassword() throws Exception {
        ...
        final String customerId = UUID.randomUUID().toString();
        customerEntity.setUuid(customerId);
        String[] encrptedPassword = passwordCryptographyProvider.encrypt("oldPwd");
        customerEntity.setSalt(encrptedPassword[0]);
        customerEntity.setPassword(encrptedPassword[1]);

        when(mockCustomerService.getCustomer("auth")).thenReturn(customerEntity);
        when(mockCustomerService.updateCustomerPassword("oldPwd", "Qawsedrf@123", customerEntity))
                .thenReturn(customerEntity);
        mockMvc
                .perform(put("/customer/password")
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                        .header("authorization", "Bearer auth")
                        .content("{\"old_password\":\"oldPwd\", \"new_password\":\"Qawsedrf@123\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("id").value(customerId));
        verify(mockCustomerService, times(1)).getCustomer("auth");
        verify(mockCustomerService, times(1)).updateCustomerPassword("oldPwd", "Qawsedrf@123", customerEntity);
    }
